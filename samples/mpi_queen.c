/* Fully optimized solution to the N-queens problem.
   ...
   Language:  C
   Taken from: http://penguin.ewu.edu/~trolfe/MpiQueen/
*/
#define DEBUG 1
#include <stdio.h>
#include <stdlib.h>
...
enum{FALSE,TRUE} TRACE=TRUE;// Enable/disable tracing execution
...
// The server sends a size of zero as a termination message
   ...
   // CRITICAL:  mark [0] as used, and then as unused
      ...
      if (TRACE) printf("%d calling Nqueens\n", myPos); fflush(stdout);
   ...
   // Put the data into the communication vector
      Count[0] = Nunique;  Count[1] = Ntotal;
